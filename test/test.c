...
void test_ft8_tones(const char* message_text, const char* expected_tones, ftx_callsign_hash_interface_t *hash_if)
{
    printf("Testing FT8 tones [%s]\n", message_text);
    ...
    char tones_text[FT8_NN + 1];
    for (int idx = 0; idx < FT8_NN; ++idx)
    {
        tones_text[idx] = '0' + tones[idx];
    }
    tones_text[FT8_NN] = '\0';

    printf("Encoded tones [%s]\n", tones_text);
    CHECK(strlen(expected_tones) == FT8_NN);
    CHECK(0 == strcmp(expected_tones, tones_text));
    TEST_END;
}
...
int main()
{
    ...
    test_std_msg("W9XYZ", "K1ABC", "R FN42");
    test_msg("W9XYZ K1ABC R FN42", "W9XYZ K1ABC R FN42", &hash_if);
    test_msg("W9XYZ K1ABC r FN42", "W9XYZ K1ABC R FN42", &hash_if);
    // Golden vector generated by WSJT-X 2.7.0-rc3 ft8code.
    test_encoding("W9XYZ K1ABC R FN42", "00001100001010010011101110000000010011011110111100011010101010100001100110001", &hash_if);
    test_ft8_tones("W9XYZ K1ABC R FN42", "3140652020355725005476704636021536313140652347403034237574055532456665023140652", &hash_if);
